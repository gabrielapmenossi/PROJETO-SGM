...
if ($conn->query($sql)) {
    $id_chamado = (int) $conn->insert_id;

    if (
        isset($_FILES['foto']) &&
        is_array($_FILES['foto']) &&
        (int) $_FILES['foto']['error'] === UPLOAD_ERR_OK
    ) {
        ...
        $nome_arquivo = 'abertura_' . uniqid('', true) . '.' . $extensao;
        $caminho_final = $diretorio . $nome_arquivo;
        if (move_uploaded_file($_FILES['foto']['tmp_name'], $caminho_final)) {
            $caminho_db = $conn->real_escape_string('assets/uploads/' . $nome_arquivo);
